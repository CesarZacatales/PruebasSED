...
middlewares.authentication = async (req, res, next) => {
  try {
    ...
    // Paso 02: Verificar que sea un token válido
    // Prefijo Token -> Bearer token_aqui
    ...
    // Paso 04: Verificar si el token está registrado en el usuario
    const isTokenValid = user.tokens.includes(token);
    if (!isTokenValid) {
      return res.status(401).json({ error: "No autorizado" });
    }

    // Paso 05: Modificar la req para tener la información del usuario
    req.user = user;
    req.token = token;
...
middlewares.authorization = (roleRequired = ROLES.SYSADMIN) => {
  return async (req, res, next) => {
    try {
      // Paso 0: Asumir que se ejecuta después del proceso de autenticación
